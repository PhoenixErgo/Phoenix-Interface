...
const fetchTokenMetadata = async (
  addresses: string[],
  explorerApiClient: any,
  usdOracle: number
): Promise<Token[]> => {
  const balances: TokenBalance[][] = await Promise.all(
    addresses.map(
      async (address) =>
        (await getTotalBalance(explorerApiClient, address)) as unknown as TokenBalance[]
    )
  );
  ...
  const walletTokenBalance = consolidatedBalance.filter((asset) => asset.tokenId !== 'ERG');
  ...
  return await Promise.all(
    walletTokenBalance.map(async (asset) => {
      ...
      const tokenUSDValue = getERGUSD(totalTokenERGValue, usdOracle);
...
export async function syncErgBalance(
  adddress: string[],
  explorerClient: any,
  usdOracle: number,
  setErgBalance: React.Dispatch<React.SetStateAction<string>>,
  setErgUSDValue: React.Dispatch<React.SetStateAction<string>>
): Promise<void> {
  const balances = await Promise.all(
    adddress.map(async (address: string) => {
      return BigInt(
        (await explorerClient.getApiV1AddressesP1BalanceConfirmed(address)).data.nanoErgs
      );
    })
  );
...
export async function syncAssetBalance(
  ...
  explorerApiClient: any,
  usdOracle: number,
  setWalletAssets: React.Dispatch<React.SetStateAction<Token[]>>
): Promise<void> {
