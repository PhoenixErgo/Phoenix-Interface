...
export async function rateLimitedCoinGeckoERGUSD(): Promise<() => Promise<number>> {
  let timestamp = 0;
  let price = 0;
...
export async function getShortLink(
  ...
  try {
    const res = await axios.get(
      `${NEXT_PUBLIC_NEST_API_URL(isMainnet)}/ergopay/generateShortLink/${base64Txn}`
    );
...
export async function getInputBoxes(
  ...
  if (tokens) {
    ...
    while (true) {
      const boxes = (await explorerApi.getApiV1BoxesUnspentByaddressP1(inputAddress, offset, limit))
        .data;
      ...
      if (Object.values(tokenInputs).every((amount, index) => amount >= tokens![index].amount)) {
  ...
  let cumInputValue: bigint = inputs.reduce((acc, curr) => acc + BigInt(curr.value), BigInt(0));
