...
const Refund = () => {
  ...
  const handleClick = async () => {
    ...
    const transactionInfo: TransactionInfo | ErgoTransaction = await getUnConfirmedOrConfirmedTx(txId, isMainnet);
    ...
    const proxyOutputs: ErgoTransactionOutput[] = 'blockId' in transactionInfo ? (transactionInfo as TransactionInfo).outputs!.map(outputInfoToErgoTransactionOutput).filter(proxyFilter) : (transactionInfo as ErgoTransaction).outputs.filter(proxyFilter);
    ...
    const receiverOutputs = proxyOutputs.filter((box) => {
      ...
      if(isBoxSpend && box.ergoTree !== changeErgoTree && box.ergoTree !== FEE_CONTRACT){
        spentCounter++;
      }
    ...
    if(receiverOutputs.length === 0){
      ...
      toast.warn('please ensure correct address is connected!', noti_option_close('try-again'));
      return;
    }
    ...
    if(walletInputSum > 0){
      const walletInputs: Box<Amount>[] = (await ergo!.get_utxos()).sort((a, b) => {
      ...
      for (let i = 0; i < walletInputs.length; i++) {
        ...
        walletInputs[i].assets.forEach(t => changeTokens.push({tokenId: t.tokenId, amount: parseInt(t.amount.toString())}));
    ...
    try{
      ...
      const inputTotalValue = inputs.reduce((acc: bigint, curr) => BigInt(curr.value) + acc, BigInt(0));
      const proxyInputTotalValue = proxyInputsWithTokens.reduce((acc: bigint, curr) => BigInt(curr.value) + acc, BigInt(0));

      const tokens: TokenAmount<Amount>[] = [];

      inputs.forEach(box => box.assets.forEach(t => tokens.push({tokenId: t.tokenId, amount: t.amount})));
      const out = tokens.length > 0 ? new OutputBuilder(inputTotalValue - BigInt(minerFee), changeAddress).addTokens(tokens) : new OutputBuilder(inputTotalValue - BigInt(minerFee), changeAddress);
      outputs.push(out);

      proxyInputsWithTokens.forEach(box => {
        const out = new OutputBuilder(box.value, changeAddress).addTokens(box.assets.map(t => ({tokenId: t.tokenId, amount: t.amount})));
        outputs.push(out);
      });

      const outputTotalValue = outputs.reduce((acc: bigint, curr) => BigInt(curr.value) + acc, BigInt(0));

      const diff = (outputTotalValue + BigInt(minerFee)) - (inputTotalValue + proxyInputTotalValue);

      if(diff > 0){
        toast.dismiss();
        toast.warn(`Wallet needs ${parseInt(diff.toString()) * 10 ** -9} more ERG`, noti_option_close('try-again'));
        return;
      }
      ...
      try{
        ...
        if(error!.info === 'Loading context data...'){
          toast.warn('contract error, ensure correct address connected', noti_option_close('try-again'));
          return;
        }
        // @ts-ignore
        if('code' in error){
      ...
      console.log(error);
      toast.dismiss();
      toast.warn('tx failed, please contact support', noti_option_close('try-again'));
    }
