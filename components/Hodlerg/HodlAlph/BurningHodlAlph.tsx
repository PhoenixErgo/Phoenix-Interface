...
const BurningHodlAlph = (props: IProps) => {
  ...
  useEffect(() => {
    if (
      !isNaN(burnAmount) &&
      !hasDecimals(burnAmount, decimals)
    ) {
  ...
  const handleClick = async () => {
    ...
    if(signer){
      ...
      let result;
      try{
        result = await burn(signer, contractId, burnAmountBigInt, HODL_ALPH_UI_FEE(burnAmountBigInt));
      } catch (error){
        toast.dismiss();
        ...
        <article className="w-full mx-auto font-inter h-1/2 p-1">
          <div className="flex flex-col md:flex-row bg-gray-200 justify-start align-start md:justify-between rounded-md items-start md:items-center h-full">
            ...
            <button
                ...
                onClick={handleClick}
            >
              BURN <br />HODL{`${props.baseTokenTicker} ${props.percent}%`}
            </button>
          </div>
        </article>
      </>
