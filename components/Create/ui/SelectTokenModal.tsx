...
const SelectTokenModal: React.FC<SelectTokenModalProps> = ({
  ...
  useEffect(() => {
    ...
    switch (network) {
      ...
      case '4': {
        axios
          .get('https://raw.githubusercontent.com/alephium/token-list/master/tokens/mainnet.json')
          .then((res) => {
            const alephiumTokens: CreationToken[] = res.data.tokens.map(
              (item: { id: string; symbol: string; name: string; logoURI: string }) => {
                return {
                  tokenId: item.id,
                  ticker: item.symbol,
                  tokenName: item.name,
      ...
      case '5': {
        axios
          .get('https://raw.githubusercontent.com/alephium/token-list/master/tokens/testnet.json')
          .then((res) => {
            const tokens: CreationToken[] = res.data.tokens.map(
              (item: { id: string; symbol: string; name: string; logoURI: string }) => {
                return {
                  tokenId: item.id,
                  ticker: item.symbol,
