...
const CreateForm = (props: IProps) => {
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        if(signer){
            ...
            let result;
            try{
                const tokenMetadata = await signer.nodeProvider!.fetchFungibleTokenMetaData(createFormData.tokenId)
                ...
                result = await create(signer, network, fee, createFormData.tokenId, tokenMetadata.decimals, createFormData.symbol, createFormData.name, BigInt(tokenMetadata.totalSupply.toString()), bankFee, creatorFee, BigInt(CREATE_UI_FEE))
            } catch (error){
                toast.dismiss();
        ...
        <>
            <div className="max-w-md mx-auto font-inter">
                ...
                <p className="text-black my-3 min-h-[100px]">
                    Create any Hodl token game! There is a creation fee of 1 ALPH. There is a UI fee of 1 ALPH.
                    Bank fee is the mechanic which allows for price increase, it must be at least one percent and at most 50 percent.
                    Creator fee is what you, the creator, gets each time someone burns. This can be set to 0 percent and at most 20 percent.
                    There is a dev fee of 10% the bank fee.
                </p>
                ...
                <form
                    ...
                    <button disabled={false} type="submit" className="w-full focus:outline-none text-white primary-gradient hover:opacity-80 focus:ring-4 focus:ring-purple-300 font-medium rounded text-md  px-4 py-3">Create</button>
                </form>
            </div>
            <SelectTokenModal
