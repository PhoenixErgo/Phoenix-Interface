...
export default async function handler(req: NextApiRequest, res: NextApiResponse<APIResponse>) {
  const bankBox = (
    await explorerClient(true).getApiV1BoxesUnspentBytokenidP1(BANK_SINGLETON_TOKEN_ID(true))
  ).data.items![0];
  ...
  const currentPriceUI = Number((currentPrice * apiPrecisionBigInt) / UIMultiplier) / apiPrecision;

  const circulatingSupplyUI =
    Number((hodlBankContract.getHodlERG3EmissionAmount() * apiPrecisionBigInt) / UIMultiplier) /
    apiPrecision;

  const tvlUI = Number((tvl * apiPrecisionBigInt) / UIMultiplier) / apiPrecision;

  const protocolFeesCollectedUI =
    Number((hodlBankContract.getProtocolFeesCollected() * apiPrecisionBigInt) / UIMultiplier) /
    apiPrecision;
